-- Fix the get_market_dashboard function to generate proper chart data
...
BEGIN
  IF stock_ticker IS NOT NULL THEN
    ...
    chart_points AS (
      -- Generate 50 historical data points for a realistic chart
      SELECT 
  ...
  ELSE
    ...
    market_stats AS (
      SELECT 
        ...
        COUNT(*) FILTER (WHERE change_percent > 0) as gainers_count,
        COUNT(*) FILTER (WHERE change_percent < 0) as losers_count,
        ROUND(AVG(change_percent), 2) as avg_change,
        TO_CHAR(MAX(sq.timestamp), 'YYYY-MM-DD"T"HH24:MI:SS"Z"') as last_updated
      FROM latest_quotes lq
      JOIN stock_quotes sq ON lq.ticker = sq.ticker
    )
    SELECT JSON_BUILD_OBJECT(
